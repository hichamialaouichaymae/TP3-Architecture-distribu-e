...
const TTL = 30; // Temps de vie du cache en secondes
...
router.post('/', async (req, res) => {
    try {
        ...
        // 3. Invalider le cache Redis (car les données ont changé)
        await redis.del('toutes_commandes');

        // 4. Publier un message dans RabbitMQ pour le consommateur
...
router.get('/', async (req, res) => {
    try {
        ...
        // 2. Si pas en cache, lire depuis MongoDB
        const commandes = await Commande.find().sort({ date: -1 });

        // 3. Stocker le résultat dans Redis pour 30 secondes
